const CACHE_NAME='sgcm-shell-20260812-02';
const SHELL=[
  './','./index.html','./styles.css','./bridge-client.js','./app.js','./config.js','./dashboard.html',
...
self.addEventListener('fetch',event=>{
  ...
  // Nunca intercepta Apps Script, Drive ou qualquer origem externa.
  if(url.origin!==self.location.origin)return;
  const isNavigation=event.request.mode==='navigate',path=url.pathname;
  ...
  if(path.endsWith('.js')||path.endsWith('.css')||path.endsWith('.webmanifest')){
    event.respondWith(staleWhileRevalidate(event.request));return;
  }
